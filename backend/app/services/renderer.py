...
from backend.app.config import DEFAULT_MUSIC_VOLUME, OUTPUT_FPS, OUTPUT_HEIGHT, OUTPUT_WIDTH
...
def _render_scene(source: Path | None, output: Path, duration: float) -> None:
    ...
    if source is None:
        run([
            "ffmpeg", "-y", "-f", "lavfi", "-i", f"color=c=0x070b12:s={OUTPUT_WIDTH}x{OUTPUT_HEIGHT}:r={OUTPUT_FPS}",
    ...
    if is_image(source):
        cmd = [
            "ffmpeg", "-y", "-loop", "1", "-framerate", str(OUTPUT_FPS), "-i", str(source),
...
def render_video(
    ...
    if music_path:
        ...
        cmd = [
            "ffmpeg", "-y", "-i", str(concat_video), "-i", str(narration_path),
